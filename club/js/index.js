...
Vue.component('club-item', {
    ...
    template: '<tr class="row"><td class="col l2 m2">{{ name }}</td><td class="col l2 m2">{{ id }}</td><td class="col l2 m2">{{ username }}</td><td class="col l6 m6"><a class="reset-pwd" href="#!" :data-id=id>重置密码</a><a href="#!" :data-id=id class="delete-club">删除部门</a></td></tr>'
});
let data;
axios.get(getUrl(`/department/list`))
    .then((response) => {
...
document.querySelector("#delete-btn").addEventListener("click", () => {
    ...
    postData(dom, "/department/delete", {
        id: dom.value
...
document.querySelector("#search-btn").addEventListener("click", () => {
    ...
    if (!check(dom)) return;
    $('#loading').modal('open');
    axios.get(getUrl('/department/search'), {
        comName: dom.value
    }).then((response) => {
        console.log(response);
        if (response.status != 200) {
        ...
        document.querySelector("#result-id").innerHTML = club.comId;
